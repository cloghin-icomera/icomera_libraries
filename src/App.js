...
const NewButton = React.forwardRef((props, ref) => {
	...
	if (props.icon && props.label) {
		icon = React.cloneElement( props.icon, { mr: 2, ml: -1 } );
	} else if( props.icon ) {
		icon = React.cloneElement( props.icon, { mr: -1, ml: -1 } );
	}
				...
				<Flex sx={{ flexWrap: 'wrap', height: '100%', overflow: 'hidden' }}>
					...
					<Flex sx={{ height: '100%' }}>
						...
						<Box p={6} sx={{ overflowY: 'scroll' }}>
							...
							<Flex mb={5} sx={{ width: '100%', justifyContent: 'space-between' }}>
								<NewButton label='Test'/>
								<NewButton
									icon={<AddCircle />}
									label='Button Label'
